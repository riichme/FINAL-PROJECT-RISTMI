...
// Import Actions
import { addItem, clearItem } from '../../app/Features/Cart/actions';
import { set_null } from '../../app/Features/Counter/actions';
import { fetchProduct, goToNextPage, goToPrevPage, setPage } from '../../app/Features/Product/actions';
...
const Home = () => {
  ...
  // State
  // This two States are used to trigger showing modal of Login and Register with the initial value false
  // If true then component Login or Register Modal will be mounted
  ...
  let coba = products.data.map((product, i) => {return product.status})
  console.log(coba.status, "<<<<")
  ...
  return (
    <div>
            ...
            {products.data.length > 0 && products.data.map((product, i) => {
                return (
                  <div key={i}>
                    <ProductCard 
                      title={product.name}
                      imgUrl={`http://localhost:3000/images/products/${product.image_url}`}
        ...
        : 
          <Login showModal={showModal} setModal={setShowModal} setRegister={setShowRegister} />
        }
    </div>
  )
}
