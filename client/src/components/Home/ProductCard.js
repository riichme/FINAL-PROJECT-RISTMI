...
// Import Action
import { inc } from '../../app/Features/Counter/actions';

// Receive Props from page Home
const ProductCard = ({title, imgUrl, price, desc, tag, modal, addToCart}) => {
  // Initial the useSelector from redux
  ...
  const handleClick = () => {
    ...
    user === null ? modal(true) : dispatch(inc(1)) && addToCart();
  }
  return (
    <div className="max-w-[17rem] mr-3 my-2  bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
        <div>
            <img className="rounded-lg mx-auto mt-4 w-24 h-24" src={imgUrl} alt="productmage" />
        </div>
        <div className="p-5">
            ...
            <button onClick={handleClick} className="w-[14rem] inline-flex items-center py-2 pl-16 text-sm font-medium text-white bg-[#149BFC] rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300">
                Add to Cart
