            ...
            <div key={i} className='flex flex-col'>
              <div className="flex m-4">
                <div className="flex items-center h-5">
                  <input name='address' onChange={handleChange} type="radio" className="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300 focus:ring-blue-500" value={alamat._id} id={alamat.nama}/>
                </div>
                <div className="ml-3 text-sm">
                    <label htmlFor={alamat.nama} className="font-medium text-gray-900 ">{alamat.nama}</label>
                    <div className="text-gray-500 "><span className="text-xs font-normal">{alamat.provinsi}, {alamat.kabupaten}, {alamat.kecamatan}, {alamat.kelurahan}, {alamat.detail}
                    </span>
                    </div>
                </div>
              </div>
