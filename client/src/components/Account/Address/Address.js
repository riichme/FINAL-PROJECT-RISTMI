...
// Address receives props from page Account
const Address = ({addressData, setOption, setShowForm}) => {
  ...
  return (
    <div>
        ...
        { showAddressForm && <AddressForm option={setOption} setShowForm={setShowAddressForm} isFormNotShowing={setShowForm}  />}
        {/* If the state address form is inactive, react just render button add address and table of address data */}
        { !showAddressForm && (
            <>
            <button onClick={() => setShowAddressForm(true)} type="button" className="w-[10rem] focus:outline-none text-white bg-blue-600 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium text-sm px-5 py-2.5 mb-2 rounded">Tambah Alamat</button>
        <div className='m-3'>
                        ...
                        <tbody key={i}>
                            <tr className="bg-white border-b">
                                ...
                                <td className="pl-16 py-3">
                                    {data.provinsi}, {data.kabupaten}, {data.kecamatan}, {data.kelurahan}, {data.detail}
                                </td>
                            </tr>
                        </tbody>
                    )
